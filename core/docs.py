...
class DocClaim:
    ...
    # 陈述类型（DESIGN §7.6）：事实陈述/事后归因/前瞻判断。只有"事实陈述"型
    # 能被 thesis.doc_pattern_triggers() 转成 R2 结构做历史验证——它描述的是
    # 已发生的联动，可以直接当"条件→观察"这对假设，不含需要独立验证的推理。
    陈述类型: str = ""
    # 仅当观点**明确写出两个标的各自的涨跌方向**时才非空（如"半导体跌15%，
    # 消费同期涨8%"）。标的名须是常见板块/指数叫法，方向取"涨"/"跌"。
    条件标的: str = ""
    条件方向: str = ""
    观察标的: str = ""
    观察方向: str = ""
...
8. **给每条观点判断"陈述类型"**，三选一：
   - **事实陈述**：描述已发生的事，如"今日大盘涨2.1%，新能源板块涨3.4%"。
   - **事后归因**：先看到结果再找原因，如"新能源大涨，因为储能招标超预期"。
   - **前瞻判断**：对未来的预测，如"预计新能源明年维持高增"。
   拿不准就填"事实陈述"以外的类型（宁可漏判为不可用，不可把归因/预测误判成事实）。
9. **仅当观点同时明确写出两个板块/指数各自的涨跌方向时**，额外填
   `条件标的`/`条件方向`/`观察标的`/`观察方向`（方向只能填"涨"或"跌"）——
   例如"半导体跌15%的同时，消费板块涨8%"→ 条件标的=半导体、条件方向=跌、
   观察标的=消费、观察方向=涨。标的名用板块/指数的常见叫法（如"半导体""消费"
...
def _verify_relation(c: DocClaim, evidence: str) -> None:
    """联动关系四元组的机械校验：条件/观察标的名必须真在原文（或表格）里出现。

    与 `_verify_chart` 对失败子部件的处理方式一致——校验不过只清空这四个字段，
    不影响观点本身的 `ok`。防的是模型编一个原文里根本没提过的板块名当"标的"。
    """
    if not (c.条件标的 and c.观察标的):
        return
    if c.条件方向 not in ("涨", "跌") or c.观察方向 not in ("涨", "跌"):
        c.条件标的 = c.条件方向 = c.观察标的 = c.观察方向 = ""
        return
    full = _norm(evidence)
    if _norm(c.条件标的) not in full or _norm(c.观察标的) not in full:
        c.条件标的 = c.条件方向 = c.观察标的 = c.观察方向 = ""
...
def _verify(c: DocClaim, pages: list[str]) -> DocClaim:
    ...
    # 联动关系四元组：与配图同等待遇，不合格只清空关系字段，不丢整条观点
    _verify_relation(c, evidence)
...
def extract(path: Path, client: DeepSeekClient | None = None,
    ...
    for it in res.data.get("观点列表", []):
        ...
        c = DocClaim(
            ...
            文档日期=out.文档日期,
            时效=out.时效,
            陈述类型=str(it.get("陈述类型", "")).strip(),
            条件标的=str(it.get("条件标的", "")).strip(),
