...
def _public_text(value: object) -> str:
    ...
    # 客户正文解释“什么变化、经什么渠道影响哪些资产”，不展示内部证据工程术语。
    text = re.sub(
        r"(?:\*\*)?(?:事件|事实)\s*[—\-→]+\s*(?:产业)?机制\s*[—\-→]+\s*A\s*股(?:对象)?暴露"
        r"(?:的)?(?:传导链)?(?:较为|已经|已)?(?:清晰|完整|成立|可验证)?(?:\*\*)?\s*[，,；;]?\s*",
        "", text, flags=re.I,
    )
    text = re.sub(r"(?:事件)?传导链(?:较为|已经|已)?(?:清晰|完整|成立|可验证)\s*[，,；;]?\s*", "", text)
    text = text.replace(
        "但需注意审厂行为尚未直接证明相关公司获得订单或收入",
        "后续重点观察供应商订单与收入兑现",
    )
    text = re.sub(
        r"单日行情不能外推为持续趋势\s*[，,]\s*需关注([^。；]+)",
        r"短期市场反应能否延续，取决于\1", text,
    )
    # 不机械删除整句风险信息，只把内部审核术语转成业务可观察条件。
    text = text.replace("证据跳跃", "后续仍需业务数据验证")
    text = text.replace("因果关系不可靠", "后续影响仍需观察")
    text = text.replace("证据链不完整", "后续仍需补充业务数据")
    text = text.replace("缺乏直接证据", "仍需后续业务数据验证")
...
3.5.4 **事件型客户正文必须用业务语言把传导关系讲明白。** 推荐顺序是：事件改变的关键变量 → 需求、价格、
产能或技术渠道 → 受影响的产业环节 → 本次行业/篮子/ETF为何承接该影响 → 投资含义。正文不得出现
“事件—机制—A股暴露”“传导链清晰/完整”“已通过证据审核”等内部流程用语。不要用“尚未直接证明获得订单或
收入”作为段落落点；若订单尚待兑现，写成“后续重点观察订单、出货或收入兑现”，且全篇最多用一句简短的观察条件。
正文篇幅应主要用于解释机会和影响渠道，不能把每段写成对传导关系的否定或免责声明。
...
4.1 图型沿用"惯用图类型"，各自的数据点填法：
   ...
   - `evidence_flow`：事件型研究中已核验的“事件事实 → 传导依据 → A股主题影响”链，
     每点 {标签, 说明}，只可写已给出的传导证据；`说明`尽量用一条不超过 40 字的短句，
     避免把整段证据和公司名单塞进图中，完整依据留在正文与底稿；它不是数值趋势图。
...
def _chart_heading(spec: dict) -> str:
    ...
    if str(spec.get("类型") or "") == "evidence_flow":
        title = re.sub(r"事件传导链$", "影响路径", title)
